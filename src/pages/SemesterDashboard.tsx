...
export const SemesterDashboard = ({
  ...
  const [editingSubjectId, setEditingSubjectId] = useState<string | null>(null); // Додано стейт для редагування предмета
  ...
  // Оновлена логіка для ЗБЕРЕЖЕННЯ (додавання і редагування) предмета
  const handleSaveSubject = () => {
    ...
    if (totalCredits - currentSubCredits + creditsNum > 30)
      return setErrorMsg(`Ліміт 30 кр.!`);

    if (editingSubjectId) {
      const updated = subjects.map((s) =>
        s.id === editingSubjectId
          ? { ...s, name: newSubName, credits: creditsNum }
          : s,
      );
      setSubjects(updated);
  ...
  const handleSaveTask = () => {
    ...
    if (newTaskType === "Курсова робота") finalMaxScore = 100;
    else {
      const currentTotal = activeSubject.tasks
        .filter((t) => t.type !== "Курсова робота" && t.id !== editingTaskId)
        .reduce((s, t) => s + t.maxScore, 0);
      if (currentTotal + finalMaxScore > 100)
        return setErrorMsg(`Макс. 100 балів!`);
    }
    const oldTask = editingTaskId
      ? activeSubject.tasks.find((t) => t.id === editingTaskId)
      : null;
    ...
    const savedTask: Task = {
      ...
      credits:
        newTaskType === "Курсова робота" ? Number(newTaskCredits) : undefined,
    };
    ...
    >
      ...
      <div className="dashboard-grid desktop-view">
        <div className="dash-left-panel">
          <div className="dash-panel-header">
            <h2>Мій Семестр</h2>
            <button
              className="dash-add-subject-btn"
              onClick={() => {
                setEditingSubjectId(null);
                setNewSubName("");
                setNewSubCredits("");
          ...
          <div className="subject-list scrollable-area">
            {subjects.map((s) => (
              ...
              >
                ...
                <div
                  className="subject-item-actions"
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "4px",
                ...
                >
                  {" "}
                  <span
                    className="subject-item-credits"
                  ...
                  <Trash
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      const u = subjects.filter((x) => x.id !== s.id);
                      setSubjects(u);
                      syncWithGlobalStorage(u);
                      if (activeSubjectId === s.id)
                        setActiveSubjectId(u[0]?.id || null);
                    }}
                  />
                </div>
              </div>
            ...
            <>
              <div className="subject-detail-header">
                <div className="subject-detail-title">
                  ...
                  <div className="subject-detail-text">
                    <h2>{activeSubject.name}</h2>
                  </div>
                </div>
                <button
                  className="dash-primary-btn-transparent"
                  onClick={() => {
                ...
                <div className="tasks-groups-list">
                  {Object.entries(
                    ...
                    return (
                      <div
                        key={name}
                        className={`task-group ${isOpen ? "is-open" : ""}`}
                      >
                        <div
                        ...
                        >
                          <div className="group-title-left">
                            <div className="group-icon-container">
                              {getGroupIcon(name as TaskType)}
                            </div>
                            <h4>{name}</h4>
                            <span className="category-score-label">
                              {tasks.reduce((s, t) => s + (t.score || 0), 0)} /{" "}
                              {tasks.reduce((s, t) => s + t.maxScore, 0)}
                            </span>
                          </div>
                          <div
                            className={`group-toggle-arrow ${isOpen ? "open" : ""}`}
                          >
                                    ...
                                    <input
                                      ...
                                      style={{
                                        width:
                                          t.score === null
                                            ? "1.2ch"
                                            : `${t.score.toString().length + 0.2}ch`,
                                      }}
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const num = Math.max(
                                          0,
                                          Math.min(Number(val), t.maxScore),
                                        );
                                        const updated = subjects.map((s) =>
                                          s.id === activeSubjectId
                                            ? {
      ...
      <div className="mobile-view">
        <div className="mobile-subjects-list">
          ...
          {/* КОНТЕНТ ЗІ СКРОЛОМ ТА ПЕРЕВІРКОЮ НА ПОРОЖНІЙ СПИСОК */}
          <div className="mobile-scrollable-content">
            {subjects.length === 0 ? (
              ...
              subjects.map((s) => (
                <div
                  key={s.id}
                  className={`mobile-subject-card ${activeSubjectId === s.id ? "expanded" : ""}`}
                  onClick={() =>
                    setActiveSubjectId(activeSubjectId === s.id ? null : s.id)
                  }
                >
                  {/* ОНОВЛЕНИЙ ХЕДЕР ПРЕДМЕТА */}
                  <div className="mobile-card-header">
                    ...
                    <div className="mobile-card-info">
                      <h3 className="mobile-card-title">{s.name}</h3>
                    </div>

                    {/* Дії: Кредити, Олівець, Кошик (Без стрілки) */}
                    <div className="mobile-card-actions">
                      <span className="mobile-card-credits-right">
                        {s.credits} кр.
                      </span>
                  ...
                  <AnimatePresence>
                    {activeSubjectId === s.id && (
                      ...
                      >
                        {/* ЗАГОЛОВКИ ТАБЛИЦІ */}
                        <div className="mobile-task-headers">
                          <div className="col-name">Назва</div>
                          <div
                        ...
                        {Object.entries(
                          ...
                          const isGroupOpen =
                            expandedGroups[`mobile-${s.id}-${typeName}`] !==
                            false; // За замовчуванням відкриті
                          ...
                          return (
                            <div key={typeName} className="mobile-task-group">
                              ...
                              >
                                ...
                                >
                                  {getGroupIcon(typeName as TaskType)}{" "}
                                  {typeName}
                                </div>
                                <CaretDown
                                  size={16}
                                  weight="bold"
                                  className={`mobile-card-arrow ${isGroupOpen ? "open" : ""}`}
                                ...
                                {isGroupOpen && (
                                  ...
                                  >
                                    {tasks.map((t) => (
                                      <div
                                        key={t.id}
                                        className="task-row mobile-task-row"
                                      >
                                        <div className="col-name">{t.name}</div>
                                        <div className="col-status">
                                          <span
                                        ...
                                        >
                                          ...
                                          {/* ОНОВЛЕНО: Додано marginLeft: "18px" */}
                                          <div
                        ...
                        <button
                          className="mobile-inner-add-btn"
                          onClick={() => {
                            setActiveSubjectId(s.id);
                            setEditingTaskId(null);
                            setIsTaskModalOpen(true);
          ...
          {/* ФІКСОВАНИЙ ПІДВАЛ */}
          <div className="mobile-credits-footer">
            Кредити: <strong>{totalCredits} / 30</strong>
          </div>
        </div>
      </div>
            ...
            >
                  ...
                  <>
                    ...
                    <div className="sem-input-group">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleNumberChange(e.target.value, setNewSubCredits)
                        }
                      />
                    </div>
                  </>
              ...
              <div className="sem-modal-actions">
                <button
                  ...
                  onClick={
                    isSubjectModalOpen ? handleSaveSubject : handleSaveTask
                  }
                >
                  Зберегти
                </button>
                <button
                  className="sem-btn-cancel"
                  onClick={() => {
                    setIsSubjectModalOpen(false);
                    setIsTaskModalOpen(false);
                    setIsTypeDropdownOpen(false);
